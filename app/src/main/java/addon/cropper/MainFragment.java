...
//            Toast.makeText(getActivity(), "Image load successful", Toast.LENGTH_SHORT).show();
    ...
    private void handleCropResult(CropImageView.CropResult result) {
        ...
        if (result.getError() == null) {
            ...
            List<tDisplayPictureData> dtList = new ArrayList<tDisplayPictureData>();
            dtList.add(tDisplayPictureData);

            new tDisplayPictureBL().saveData(dtList);

//            Bitmap bitmapFactory = BitmapFactory.decodeResource(getResources(), R.drawable.ic_error);
//            Bitmap bitmaps = Bitmap.createScaledBitmap(bitmapFactory, 600, 600, true);
//
//            ByteArrayOutputStream outs;
//
//            outs = new ByteArrayOutputStream();
//            bitmaps.compress(Bitmap.CompressFormat.JPEG, 100, out);
//            byte[] imgOnByte = outs.toByteArray();

            clsDisplayPictures pictures = new clsDisplayPictures(Integer.valueOf(tDisplayPictureData.get_intID()), pht);
            new clsDisplayPicturesBL(getActivity()).saveDataImage(pictures);
